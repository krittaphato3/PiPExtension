    ...
    try {
      if (typeof chrome !== 'undefined' && chrome.storage?.local) {
        ...
        if (state) {
          ...
          // Stale state detection: if state says open but has no pipId, clear it
...
const PiPFactory = {
  ...
  async create(options = {}) {
    ...
    if (!shouldUsePopup && videoElement) {
      ...
      // If native fails and user didn't force it, fallback to popup
      if (!result.success && !forceNative) {
        console.log('[PiPFactory] Native PiP failed/unavailable, falling back to popup');
  ...
  async _createNativePip(videoElement, options = {}) {
    ...
    try {
      ...
      const pipWidth = Math.max(PiPFactoryConfig.MIN_WIDTH, width || rect.width || PiPFactoryConfig.DEFAULT_WIDTH);
      const pipHeight = Math.max(PiPFactoryConfig.MIN_HEIGHT, height || rect.height || PiPFactoryConfig.DEFAULT_HEIGHT);

      // Request native PiP with timeout
      const pipWindow = await Promise.race([
        window.documentPictureInPicture.requestWindow({
          width: Math.round(pipWidth),
          height: Math.round(pipHeight),
        }),
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Native PiP request timed out')), PiPFactoryConfig.NATIVE_PIP_TIMEOUT_MS)
        ),
      ]);
      ...
      // ✅ LIFECYCLE SYNC: Mark native PiP as open
      await NativePipStateManager.setOpened(pipId);

      // Move the video element into the PiP window
      // Clone video element to avoid disrupting the page
      const videoClone = videoElement.cloneNode(true);
      videoClone.style.width = '100%';
      videoClone.style.height = '100%';
      videoClone.style.objectFit = 'contain';
      videoClone.style.background = 'black';
      videoClone.controls = true;
      videoClone.dataset.pipId = pipId;

      // Clear PiP window body and add video
      ...
      // Sync playback state
      this._syncVideoState(videoElement, videoClone);

      // ✅ LIFECYCLE SYNC: Listen for PiP window close
      pipWindow.addEventListener('pagehide', async () => {
        // Unregister source for dedup
        this._unregisterSource(sourceId);
        // Clear native PiP state
        await NativePipStateManager.clearState();
        console.log(`[PiPFactory] Native PiP window closed: ${pipId}`);
      });
      ...
      console.error('[PiPFactory] Native PiP failed:', e);
      ...
      if (e.name === 'NotAllowedError') {
        ...
        errorMessage = 'PiP request timed out. The browser may be busy.';
      }
    ...
    try {
      ...
      if (targetUrl.startsWith('blob:')) {
        return {
          ...
          pipId,
          method: 'popup',
          error: 'Blob URLs cannot be opened in popup windows. Use native PiP instead.',
        };
      }
   ...
   * @returns {boolean}
   */
  _shouldUsePopup({ videoElement, videoUrl, screenId, left, top, forcePopup, forceNative }) {
    ...
    if (screenId !== undefined && screenId !== null) return true;
    if (left !== undefined && left !== null) return true;
    if (top !== undefined && top !== null) return true;

    // No video element for native → must use popup
    if (!videoElement) return true;

    // ── KEY CHECK: Is a documentPictureInPicture window already open? ──
    // `documentPictureInPicture.window` is non-null while the PiP window exists.
    // This is the CORRECT check for the document PiP API (not pictureInPictureElement).
    // Guard: `window` may be undefined in service worker context.
    if (typeof window !== 'undefined' && window.documentPictureInPicture?.window) {
      console.log('[PiPFactory] documentPictureInPicture.window exists → using popup to avoid replacement');
      return true;
    }

    // Cross-tab check: another tab may have opened a native PiP
    if (NativePipStateManager.isNativePipOpen()) {
      console.log('[PiPFactory] Native PiP open in another tab → using popup');
      return true;
    }
  ...
  _syncVideoState(source, target) {
    ...
    // Listen for changes on source
    const onPlay = () => { target.play().catch(() => {}); };
    const onPause = () => { target.pause(); };
    const onSeeked = () => { target.currentTime = source.currentTime; };
    const onVolumeChange = () => {
      target.volume = source.volume;
      target.muted = source.muted;
    };

    // Listen for changes on target (sync back to source)
    const onTargetPlay = () => { if (source.paused) source.play().catch(() => {}); };
    const onTargetPause = () => { if (!source.paused) source.pause(); };
    const onTargetSeeked = () => { source.currentTime = target.currentTime; };
    const onTargetVolumeChange = () => {
      source.volume = target.volume;
      source.muted = target.muted;
    };
    ...
    target.addEventListener('play', onTargetPlay);
    target.addEventListener('pause', onTargetPause);
    target.addEventListener('seeked', onTargetSeeked);
    target.addEventListener('volumechange', onTargetVolumeChange);
    ...
    this._syncIntervals.set(syncKey, {
      ...
      listeners: {
        ...
        target: { onTargetPlay, onTargetPause, onTargetSeeked, onTargetVolumeChange },
      },
    });
  ...
  async init() {
    ...
    if (typeof chrome !== 'undefined' && chrome.windows) {
      ...
      // ✅ LIFECYCLE SYNC BACKUP: Listen for native PiP window removal
      // This is a backup in case pagehide doesn't fire
      chrome.windows.onRemoved.addListener(async (windowId) => {
        // If the state manager says native PiP is open and we get a window removal,
        // check if it was the native PiP window (we don't have a direct reference in SW)
        // This is best-effort; the primary cleanup is pagehide
        if (NativePipStateManager.isNativePipOpen()) {
          // We can't directly verify if this was the native PiP window,
          // but if the window was created by documentPictureInPicture,
          // it will have been tracked separately. For now, we rely on pagehide.
        }
      });
    }
