...
const PiPFactory = {
  ...
  async _createPopupViaBackground(options) {
    ...
    try {
      ...
      // Note: sourceTabId is automatically set by background.js from msg sender
      const result = await chrome.runtime.sendMessage({
  ...
  _shouldUsePopup({ videoElement, videoUrl, screenId, left, top, forcePopup, forceNative, mode = 'hybrid' }) {
    console.log('[PiPFactory] _shouldUsePopup called with mode:', mode, 'videoElement:', !!videoElement, 'videoUrl:', videoUrl?.substring(0, 30));
