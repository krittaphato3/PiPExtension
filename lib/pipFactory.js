...
const PiPFactory = {
  ...
  _clearSyncByKey(key) {
    ...
    if (target) {
      try {
        target.removeEventListener('play', listeners.target.onTargetPlay);
      } catch {}
      try {
        target.removeEventListener('pause', listeners.target.onTargetPause);
      } catch {}
      try {
        target.removeEventListener('seeked', listeners.target.onTargetSeeked);
      } catch {}
      try {
        target.removeEventListener('volumechange', listeners.target.onTargetVolumeChange);
      } catch {}
    }
