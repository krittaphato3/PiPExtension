...
const PiPFactory = {
  ...
  async closePopup(idOrSelector) {
    ...
    if ((windowId === undefined || windowId === null) && (pipId || sourceId)) {
      for (const [wid, data] of this.popupWindows.entries()) {
        ...
        if (
          sourceId &&
          (mapped === sourceId || (data && (data.sourceId === sourceId || data.videoUrl === sourceId)))
        ) {
          windowId = wid;
          break;
        }
    ...
    if (typeof chrome === 'undefined' || !chrome.windows) {
      try {
        ...
        if (!localSource) {
          ...
          localSource =
            this._popupWindowSources.get(windowId) || (tracked && tracked.sourceId);
        }
  ...
  async _getUserMaxPopups() {
    try {
      ...
      const sync = stores[1] || {};
      const raw =
        local.maxPipWindows !== undefined ? local.maxPipWindows : sync.maxPipWindows;
