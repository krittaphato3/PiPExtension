...
const PiPFactory = {
  ...
  async _createNativePip(videoElement, options = {}) {
    ...
    // ✅ CRITICAL FIX: For blob URLs, use standard video.requestPictureInPicture()
    // because documentPictureInPicture clones the video element, which breaks blob URLs
    // (blob URLs are scoped to the original document).
    const isBlobUrl = this._isBlobUrl(videoElement);
    console.log('[PiPFactory] Video is blob URL:', isBlobUrl);

    if (isBlobUrl) {
      console.log('[PiPFactory] Using standard video.requestPictureInPicture() for blob URL');
      try {
        // Request standard PiP (browser handles blob URLs correctly)
        ...
        console.error('[PiPFactory] Standard PiP failed for blob URL:', e);
        return {
          success: false,
          pipId,
          method: 'native',
          error: e.message || 'Native PiP request failed for blob URL',
        };
      }
    }

    // For non-blob URLs, use documentPictureInPicture (cloning works fine)
    try {
      // Set dimensions
      ...
      // ✅ FIX: Explicitly try to play the cloned video
      // This ensures video starts playing even if sync doesn't trigger it
      if (!videoElement.paused) {
        videoClone.play().catch(e => {
          console.debug('[PiPFactory] Auto-play prevented in native PiP:', e.message);
        });
      }
    ...
    try {
      ...
      if (targetUrl.startsWith('blob:')) {
        ...
        return {
          ...
          pipId,
          method: 'popup',
          error: 'This video uses a blob URL which cannot be opened in popup windows. Please use native PiP mode instead.',
        };
      }
