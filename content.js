...
document.addEventListener("mousedown", (e) => {
  if (State.isPickerActive) return; // Let picker handle clicks
...
function handlePickerClick(e) {
  ...
  togglePickerMode(); // Turn off picker
...
async function launchElementPiP(sourceNode) {
  ...
  try {
    ...
    Array.from(document.styleSheets).forEach(styleSheet => {
      ...
      } catch (e) { /* CORS or restricted styles */ }
    });
    ...
    // Remove fixed positioning if present to ensure it's visible
...
async function launchImagePiP() {
  ...
  try {
    ...
    if (target.tagName === 'CANVAS') {
      ...
      // 4. Optimized Sync (Debounced 50ms) - Only for images
...
// --- Logic: Styling & UI ---
function setupPipStyles(doc, sourceNode) {
  let bgColor = '#000';
  try {
  ...
  style.textContent = `
    body { 
      margin: 0; background: ${bgColor}; height: 100vh; 
...
function setupZoomAndPan(img) {
  ...
  const updateTransform = () => {
    ...
    // Since translate is inside the scaled coordinate system, 
    // visual shift = translation * scale.
    // We divide delta by scale during calculation to compensate, ensuring 1:1 screen movement.
  ...
  img.addEventListener('pointermove', (e) => {
    ...
    // Adjust for scale to maintain 1:1 cursor tracking
